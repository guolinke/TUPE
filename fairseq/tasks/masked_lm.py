        ...
        if bpe is not None:
            def is_end_of_word(i):
                ...
                if tok.startswith('madeupword'):
                    return True
                try:
                    return bpe.is_end_of_word(tok)
                except ValueError:
                    return True

            token_is_end_of_word = torch.ByteTensor(list(
                map(is_end_of_word, range(len(self.source_dictionary)))
            ))

        # create masked input and targets
        if self.args.mask_whole_words:
            assert bpe is not None
            mask_whole_words = token_is_end_of_word
        else:
